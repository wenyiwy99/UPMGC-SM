...
%
function [S0,obj] = GMC(X, c, lambda, normData)
%% input:
...
% Code-Reference:
% Hao Wang:  [GMC: Graph-based Multi-view Clustering]

NITER = 20;
zr = 10e-11;
% number of neighbours for constructS_PNG
...
for v = 1: m
    for i = 1 : num
        for j = 1 : num
            %obj = obj+ed{v}(i,j)*S0{v}(i,j)+S0{v}(i,j)^2+w(v)*(U(i,j)-S0{v}(i,j))^2;
            obj = obj+ed{v}(i,j)*S0{v}(i,j);
        end
    end
end
%obj = obj + 2*lambda*trace(F'*L*F);
