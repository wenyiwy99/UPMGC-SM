...
if ischar(start)
    ...
    j = find(strncmpi(start,startNames,length(start)));  %只比较前6个字符，意思是以sample开头的都行
    if length(j) > 1
...
for t=1:reps %挑选锚点次数
    switch start
        case 'sample'
            center = X(randsample(n,k),:); %选出来的锚点坐标
        case 'cluster'
    ...
    switch distance
        case 'sqeuclidean'
            %有任何label~=last，返回true，矩阵的话返回逻辑行向量（判断一列有没有）
            while any(label ~= last) && it<maxit
                last = label;
                %full将稀疏矩阵变为满存储
                bb = full(sum(center.*center,2)'); %sum取2表示对行求和，返回列向量
                ab = full(X*center');
                D = bb(ones(1,n),:) - 2*ab; %取bb的第一行取n次，
                %得出的矩阵为n*k维的矩阵，反映的是n个数据点离k个中心/锚点的距离
...
if mod(nargs,2)~=0
    ...
    for j=1:2:nargs
        ...
        if isempty(i)
            ...
            % one to the list
            if nargout > nparams+2  %如果输出的参数大于pname的参数个数加2
